...
class PeleeNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=[3, 4, 8, 6],
                 num_init_features=32, bottleneck_width=[1, 2, 4, 4], drop_rate=0, num_classes=1000):
...
def load_model(pretrained_model_path, model_classes=1000, data_classes=15):
    model = PeleeNet(num_classes=model_classes)
    model_dict = model.state_dict()

    pretrained_model = torch.load(pretrained_model_path)['state_dict']
    #pretrained_model = torch.load(pretrained_model_path)
    #print(pretrained_model)
    pretrained_dict = {k.strip('module.'): v for k, v in pretrained_model.items() if k.strip('module.') in model_dict}
    ...
    if not model_classes == data_classes:
        model.classifier = nn.Linear(model.classifier.in_features, data_classes)    
    #print(model.state_dict())

    return model


def save_step(state, is_best, mask, filename='weights/checkpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, mask + 'model_best.pth.tar')
